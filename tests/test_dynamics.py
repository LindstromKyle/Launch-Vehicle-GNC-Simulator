...
def test_calculate_dynamics_gravity_only(basic_environment, basic_vehicle):
    ...
    deriv = calculate_dynamics(
        state, basic_vehicle, basic_environment, 0.0, controls, False
    )
    accel = deriv[3:6]
...
def test_calculate_dynamics_thrust(basic_environment, basic_vehicle):
    ...
    controls = {
        "throttle": 1.0,
        "engine_gimbal_angles": [[0, 0]],
        "rcs_levels": [],
        "desired_torque": np.zeros(3),
    }
    deriv = calculate_dynamics(
        state, basic_vehicle, basic_environment, 1.0, controls, False
    )
    accel = deriv[3:6]
